...
class Gitea(GiteaBase):
    ...
    def __init__(self, username, password):
        try:
            res = requests.get(f'{self.API_BASE_URL}/admin/users/',
...
class Org(GiteaBase):
    ...
    def create_repo(self, name, private, source=None, code=None, default_branch='main', collaborators=None,
...
class Repo(GiteaBase):
    ...
    def push_code(self, git_repo_path, source):
        try:
            if source:
                git.Repo.clone_from(source, git_repo_path)
                os.rename(f'{git_repo_path}/.git',f'{git_repo_path}/no.git')
                repo = git.Repo.init(git_repo_path)
                repo.index.add("*")
                repo.index.add(".github/*")
                origin = repo.create_remote("origin", f'{self.BASE_URL}/{self.org}/{self.name}.git')
                repo.index.commit("Secure the bank ")
